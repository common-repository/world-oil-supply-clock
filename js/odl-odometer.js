...
//  Oil Depletion Clock Widget for WordPress
//  https://www.peterleeds.com/oil-clock.htm
//  Version 1.0
//  Copyright (C) 2016 Robert Miller
...
function OdlOdometer(container) {
	if (!container) {
		throw "ERROR: OdlOdometer object must be passed a document element.";
	}
	...
	// these will override styles in odl-counter.css
	this.style = {
	...
	this.highlights = [
		"odl-highlight-1 odl-highlight",
		"odl-highlight-2 odl-highlight",
		"odl-highlight-3 odl-highlight",
		"odl-highlight-4 odl-sidehighlight",
		"odl-highlight-5 odl-sidelowlight",
		"odl-highlight-6 odl-lowlight",
		"odl-highlight-7 odl-lowlight",
		"odl-highlight-8 odl-lowlight"
	];
	...
	this.drawOdometer = function(container) {
    	...
    	case 'left':
        container.className += ' odl-left';
        break;
    	case 'right':
        container.className += ' odl-right';
        break;
    	case 'inline':
        container.className += ' odl-inline';
        break;
    	default:
        container.className += ' odl-center';
		}
		var odometerDiv = document.createElement("div");
		odometerDiv.className = "odl-odometer-div";
		...
		if (this.tenths) {
			this.digitInfo[this.digits - 1].digitA.className = "odl-tenth";
			this.digitInfo[this.digits - 1].digitB.className = "odl-tenth";
		}
	...
	this.updateOdometer = function() {
		if (this.timestamp) {
			this.currentValue = (this.direction == 'down') ?
				this.currentValue - this.iterationValue : this.currentValue + this.iterationValue;
